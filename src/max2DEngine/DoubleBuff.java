...
public class DoubleBuff extends Canvas{
	...
	public ArrayList<TextOnScreen> texts;
	...
	private BasicStroke defaultStrokeWidthObject;
	...
	public DoubleBuff(int size_x, int size_y, String name, Color back) {
		...
		defaultStrokeWidthObject = new BasicStroke(1);
	...
	protected void draw() {
		...
		g.setStroke(defaultStrokeWidthObject);
		...
		g.setColor(Color.black);
		for(Line l:lines) {
			g.setStroke(new BasicStroke(l.strokeWidth));
			g.setColor(l.color);
			//g.setPaintMode();
		...
		for(Entity entity:entitys) {
			...
			g.setStroke(defaultStrokeWidthObject);
			g.setColor(Color.black);
			for(TextOnScreen text:entity.texts) {
				g.drawString(text.s, text.x, text.y);
			}
		}
		g.dispose();
	...
	public static class TextOnScreen {
		...
		public TextOnScreen(String s, Coord2D position) {
			this.x = position.gX();
			this.y = position.gY();
			this.s = s;
		}
	...
	public static class Line {
		...
		public Line(int x, int y, int x2, int y2, Color color, int strokeWidth) {
			this.x = x;
			this.y = y;
			this.x2 = x2;
			this.y2 = y2;
		...
		public static Line fromCoord2D(Coord2D firstPoint, Coord2D secondPoint, Color color) {
			return new Line(firstPoint.gX(), firstPoint.gY(), secondPoint.gX(), secondPoint.gY(), color);
		}
		public static Line fromCoord2D(Coord2D firstPoint, Coord2D secondPoint, Color color, int strokeWidth) {
			return new Line(firstPoint.gX(), firstPoint.gY(), secondPoint.gX(), secondPoint.gY(), color, strokeWidth);
		}
